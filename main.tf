...
resource "local_file" "libvirt_br_wan_xml" {
  ...
  content  = chomp(templatefile("${path.module}/br-wan.tftpl", each.value))
...
resource "local_file" "libvirt_br_lan_xml" {
  ...
  content  = chomp(templatefile("${path.module}/br-lan.tftpl", each.value))
...
resource "local_file" "libvirt_br_mgmt_xml" {
  ...
  content  = chomp(templatefile("${path.module}/br-mgmt.tftpl", each.value))
...
resource "local_file" "libvirt_vm_xml" {
  ...
  content  = chomp(templatefile("${path.module}/vm.tftpl", each.value))
...
resource "local_file" "startup_script" {
  ...
  content  = chomp(templatefile("${path.module}/startup-sh.tftpl", each.value))
...
resource "local_file" "frr_conf" {
  ...
  content  = chomp(templatefile("${path.module}/frr_conf.tftpl", each.value))
...
resource "local_file" "libvirt_hook_network" {
  ...
  content  = chomp(templatefile("${path.module}/libvirt-hook-network.tftpl", each.value))
